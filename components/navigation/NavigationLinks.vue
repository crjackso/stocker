    ...
    <template v-for="link in links" :key="link.name">
      ...
      >
        <template #activator>
          ...
          <active-icon
            :icon="link.icon"
            :active="hasActiveSubnav(link.name)"
            data-test-nav-icon
          />
          hi
          <!-- </v-list-item-icon> -->
...
export default {
  ...
  methods: {
    ...
    subroutes(parentNavName) {
      const parentNav = this.links.filter(
        (link) => link.name === parentNavName
      )[0]
      if (!parentNav) return []
      return parentNav.sublinks.map((link) => link.to)
    },
    hasActiveSubnav(parentNavName) {
      return this.subroutes(parentNavName).some((route) =>
        this.hasActiveChildRoute(route)
      )
    }
  }
}
...
:deep(
    .active-parent-nav.v-list-group
